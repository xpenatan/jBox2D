package com.github.xpenatan.box2d.gdx.gl;
...
/** Collects Box2D debug-draw callbacks and renders them with libGDX OpenGL utilities. */
public class GdxGlDebugRenderer extends B2DebugDrawEm {
    ...
    private final boolean ownsGraphics;
    ...
    public GdxGlDebugRenderer() {
        this(new ShapeRenderer(), new SpriteBatch(), new BitmapFont(), new BitmapFont(), true);
    }

    /** Uses caller-owned rendering resources. They remain the caller's responsibility to dispose. */
    public GdxGlDebugRenderer(ShapeRenderer shapes, SpriteBatch batch,
                              BitmapFont worldFont, BitmapFont screenFont) {
        this(shapes, batch, worldFont, screenFont, false);
    }

    private GdxGlDebugRenderer(ShapeRenderer shapes, SpriteBatch batch,
                               BitmapFont worldFont, BitmapFont screenFont, boolean ownsGraphics) {
        if(shapes == null) throw new IllegalArgumentException("shapes cannot be null");
        if(batch == null) throw new IllegalArgumentException("batch cannot be null");
        if(worldFont == null) throw new IllegalArgumentException("worldFont cannot be null");
        if(screenFont == null) throw new IllegalArgumentException("screenFont cannot be null");
        this.shapes = shapes;
        this.batch = batch;
        this.worldFont = worldFont;
        this.screenFont = screenFont;
    ...
    public void render(B2World world, OrthographicCamera camera) {
    ...
    /** Renders geometry collected since the last {@link #beginFrame()} call. */
    public void render(OrthographicCamera camera) {
        render(camera, shapes, batch, worldFont, screenFont);
    }

    /** Collects and renders one world using caller-supplied rendering resources. */
    public void render(B2World world, OrthographicCamera camera, ShapeRenderer shapes, SpriteBatch batch,
                       BitmapFont worldFont, BitmapFont screenFont) {
        if(world == null) throw new IllegalArgumentException("world cannot be null");
        beginFrame();
        DrawWorld(world);
    ...
    public void render(OrthographicCamera camera, ShapeRenderer shapes, SpriteBatch batch,
                       BitmapFont worldFont, BitmapFont screenFont) {
    ...
    protected void onNativeDispose() {
        if(graphicsDisposed || !ownsGraphics) return;
