...
public final class WheelJointSample extends AbstractBox2DSample {
    ...
    public WheelJointSample() {
        ...
        B2Body body = createDynamicBody(0.0f, 10.25f, 0.0f);
    ...
    public List<Box2DSampleControl> controls() {
        ...
        c.add(Box2DSampleControl.checkbox("Limit", () -> enableLimit ? 1 : 0,
                value -> { enableLimit = value != 0; joint.WheelEnableLimit(enableLimit); joint.WakeBodies(); }));
        c.add(Box2DSampleControl.checkbox("Motor", () -> enableMotor ? 1 : 0,
                value -> { enableMotor = value != 0; joint.WheelEnableMotor(enableMotor); joint.WakeBodies(); }));
        if(enableMotor) {
            c.add(Box2DSampleControl.slider("Torque", 0, 20, .1f, () -> motorTorque,
                    value -> { motorTorque = value; joint.WheelSetMaxMotorTorque(value); joint.WakeBodies(); }));
            c.add(Box2DSampleControl.slider("Speed", -20, 20, .1f, () -> motorSpeed,
                    value -> { motorSpeed = value; joint.WheelSetMotorSpeed(value); joint.WakeBodies(); }));
        }
        c.add(Box2DSampleControl.checkbox("Spring", () -> enableSpring ? 1 : 0,
                value -> { enableSpring = value != 0; joint.WheelEnableSpring(enableSpring); joint.WakeBodies(); }));
        if(enableSpring) {
            c.add(Box2DSampleControl.slider("Hertz", 0, 10, .1f, () -> hertz,
                    value -> { hertz = value; joint.WheelSetSpringHertz(value); joint.WakeBodies(); }));
            c.add(Box2DSampleControl.slider("Damping", 0, 2, .1f, () -> dampingRatio,
                    value -> { dampingRatio = value; joint.WheelSetSpringDampingRatio(value); joint.WakeBodies(); }));
        }
        c.add(Box2DSampleControl.dynamicText(() -> String.format("Motor torque = %.1f", joint.WheelGetMotorTorque())));
        return c;
    }
}
